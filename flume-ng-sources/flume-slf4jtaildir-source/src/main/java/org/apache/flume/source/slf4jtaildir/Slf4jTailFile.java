...
public class Slf4jTailFile {
    private static final Logger       logger       = LoggerFactory.getLogger(Slf4jTailFile.class);
    ...
    public Slf4jTailFile(File file, Map<String, String> headers, long inode,
    ...
    private Event readEvent(boolean backoffWithoutNL, boolean addByteOffset) throws IOException {
        ...
        if (backoffWithoutNL && !line.lineSepInclude) {
            logger.info("Backing off in file without newline: " + path + ", inode: " + inode
    ...
    private byte[] concatByteArrays(byte[] a, int startIdxA, int lenA, byte[] b, int startIdxB,
                                    int lenB) {
    ...
    public LineResult readLine() throws IOException {
        ...
        while (true) {
            ...
            oldBuffer = concatByteArrays(oldBuffer, 0, oldBuffer.length, buffer, bufferPos,
